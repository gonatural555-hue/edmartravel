...
type WhatsAppReservationButtonProps = {
  href: string | null;
  label: string;
  unavailableMessage?: string;
};

export default function WhatsAppReservationButton({
  href,
  label,
  unavailableMessage,
}: WhatsAppReservationButtonProps) {
  if (!href) {
    return unavailableMessage ? (
      <p className="rounded-xl border border-amber-700/20 bg-amber-50 px-4 py-3 text-sm text-amber-950">
        {unavailableMessage}
      </p>
    ) : null;
  }
  ...
  return (
    <Link
      ...
      className={BOOKING_GLASS.whatsappCta}
    >
